...
description:
  ...
  standards?) and Spec (does the code match what the originating ticket/spec
...
- **Standards** — does the code conform to this repo's documented coding
  standards?
- **Spec** — does the code faithfully implement the originating tickets / spec /
  PRD?

Both axes run as parallel sub-agents so they don't pollute each other's context,
then this skill aggregates their findings.

If ticket tracker workflow is undocumented, ask the user how to fetch ticket
details.
...
1. Ticket references in the commit messages (`#123`, `Closes #45`, GitLab `!67`,
   etc.) — fetch via the workflow in `docs/agents/ticket-tracker.md`.
...
- The brief: "Report — per file/hunk where relevant — (a) every place the diff
  ...
  but baseline smells are always judgement calls, and a documented repo standard
